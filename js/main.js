...
(function ($) {
	...
	document.addEventListener("DOMContentLoaded", function () {
		document.querySelectorAll(".toggle-details").forEach(link => {
			link.addEventListener("click", function (e) {
				...
				const article = link.closest(".hentry");
				const container = article.querySelector(".recipe-details");
				const url = link.dataset.url;

				// Se já estiver visível, recolhe
				if (container.style.display === "block") {
					container.style.display = "none";
					container.innerHTML = ""; // Opcional: limpa
					return;
				}
	...
	// PAGE CONTATO : DETALHES DO FORMLÁRIO
	...
	document.getElementById('contact-form').addEventListener('submit', function (event) {
		...
		xhr.open('POST', 'send-mail.php', true); // Configura o método POST e o arquivo PHP

		// Quando a requisição for completada
		xhr.onload = function () {
			if (xhr.status === 200) {
				var response = xhr.responseText; // Resposta do PHP

				// Exibir a mensagem na tela
				var messageStatus = document.getElementById('message-status');
				if (response === 'success') {
					messageStatus.style.color = 'green';
					messageStatus.textContent = 'Mensagem enviada com sucesso!';
				} else if (response === 'unsuccess') {
					messageStatus.style.color = 'red';
					messageStatus.textContent = 'Ocorreu um erro ao enviar sua mensagem. Tente novamente.';
				} else if (response === 'error') {
					messageStatus.style.color = 'red';
					messageStatus.textContent = 'Por favor, preencha todos os campos corretamente.';
				} else {
					messageStatus.style.color = 'red';
					messageStatus.textContent = 'Erro desconhecido. Tente novamente.';
				}

				messageStatus.style.display = 'block'; // Exibe a mensagem de status
			}
		};
	...
	// PAGE AutoAvaliação : DETALHES DAS CALCULADORAS

	document.addEventListener('DOMContentLoaded', function () {

		// Função utilitária para exibir o modal e aguardar confirmação
		function mostrarModal(callback) {
			const modal = document.getElementById("modal-alerta");
			const okBtn = document.getElementById("modal-ok");
	
			modal.style.display = "flex";
	
			// Garante que o botão execute o callback apenas uma vez
			...
			mostrarModal(() => {
				...
				if (peso && altura) {
					...
					if (imc < 18.5) {
						...
						statusPeso = "Peso saudável";
					} else if (imc >= 25 && imc < 29.9) {
						statusPeso = "Sobrepeso";
					} else if (imc >= 30 && imc < 40) {
						statusPeso = "Obeso";
					} else {
						statusPeso = "Obeso mórbido";
					}
	
					document.getElementById("imc-valor").textContent = imc.toFixed(2);
					document.getElementById("imc-status").textContent = statusPeso;
					document.getElementById("imc-result-container").style.display = "block";
	
					let posicao = 0;
					if (imc < 18.5) {
						posicao = (imc / 18.5) * 15;
					} else if (imc >= 18.5 && imc < 24.9) {
						posicao = 15 + ((imc - 18.5) / (24.9 - 18.5)) * 20;
					} else if (imc >= 25 && imc < 29.9) {
						posicao = 35 + ((imc - 25) / (29.9 - 25)) * 20;
					} else if (imc >= 30 && imc < 40) {
						posicao = 55 + ((imc - 30) / (40 - 30)) * 20;
					} else {
						posicao = 75 + ((imc - 40) / 10) * 25;
						if (posicao > 100) posicao = 100;
					}
					document.getElementById("indicador-imc").style.left = `${posicao}%`;
				} else {
					document.getElementById("imc-valor").textContent = "";
					document.getElementById("imc-status").textContent = "Por favor, preencha todos os campos!";
					document.getElementById("indicador-imc").style.left = "0%";
					document.getElementById("imc-result-container").style.display = "none";
				}
			});
		};
	
		// Função para calcular TMB
		window.calcularTMB = function () {
			mostrarModal(() => {
				var peso = parseFloat(document.getElementById("tmb-peso").value);
				var altura = parseFloat(document.getElementById("tmb-altura").value);
				var idade = parseInt(document.getElementById("tmb-idade").value);
				var sexo = document.getElementById("tmb-sexo").value;
				...
				if (peso && altura && idade) {
					...
					var statusPeso = "";
					if (tmb < 1500) {
						statusPeso = "Abaixo do recomendado para a sua faixa etária e sexo";
					} else if (tmb >= 1500 && tmb < 2500) {
						statusPeso = "Dentro do intervalo recomendado";
					} else {
						statusPeso = "Acima do recomendado para a sua faixa etária e sexo";
					}
	
					document.getElementById("tmb-valor").textContent = `${tmb.toFixed(2)} kcal/dia`;
					document.getElementById("tmb-status").textContent = statusPeso;
					document.getElementById("tmb-result-container").style.display = "block";
	
					let porcentagem = Math.min(Math.max((tmb / 4000) * 100, 0), 100);
					document.getElementById("indicador-tmb").style.left = `${porcentagem}%`;
				} else {
					document.getElementById("tmb-valor").textContent = "";
					document.getElementById("tmb-status").textContent = "Por favor, preencha todos os campos!";
					document.getElementById("indicador-tmb").style.left = "0%";
				}
			});
		};
	
		// Função para calcular GET
		window.calcularGET = function () {
			mostrarModal(() => {
				var peso = parseFloat(document.getElementById("get-peso").value);
				var atividade = document.getElementById("get-atividade").value;
				...
				if (peso) {
					...
					var get = peso * 24 * fatorAtividade;
					let statusPeso = "";
	
					if (get < 1500) {
						statusPeso = "Abaixo das necessidades calóricas";
					} else if (get >= 1500 && get < 2500) {
						statusPeso = "Dentro das necessidades calóricas";
					} else {
						statusPeso = "Acima das necessidades calóricas";
					}
	
					document.getElementById("get-valor").textContent = get.toFixed(2) + " kcal/dia";
					document.getElementById("get-status").textContent = statusPeso;
					document.getElementById("get-result-container").style.display = "block";
	
					let minimo = 1000;
					let maximo = 4000;
					let porcentagem = Math.min(Math.max((get - minimo) / (maximo - minimo) * 100, 0), 100);
					document.getElementById("indicador-get").style.left = `${porcentagem}%`;
				} else {
					document.getElementById("get-valor").textContent = "";
					document.getElementById("get-status").textContent = "Por favor, preencha todos os campos!";
					document.getElementById("indicador-get").style.left = "0%";
					document.getElementById("get-result-container").style.display = "none";
				}
			});
		};
